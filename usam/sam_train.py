...
for epoch in range(num_epochs):
    ...
    for step, (image_embedding, gt2D, boxes) in enumerate(tqdm(train_dl)):
      val, counts = np.unique(masks, return_counts=True)
      if(1 - (counts[0]/counts.sum())) > 0.01:
        # image_embedding = F.interpolate(image_embedding, size=(256, 64, 64), mode='bilinear', align_corners=False)
        # image_embedding = image_embedding[:, :, :64, :64]
        # image_embedding = image_embedding.repeat(1, 256, 1, 1)
